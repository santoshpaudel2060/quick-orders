...
// //           // CRITICAL FIX: Only show orders from CURRENT SESSION
// //           // Filter by customer name AND orders created after session started
...
// //               // Initialize new session when QR is scanned
...
// //   // FIXED: Properly clear all customer data and orders after payment
...
// //       // Step 1: Delete all orders for this customer at this table from the database
// //       try {
...
// //         // Continue anyway - we'll still free the table and clear local state
// //       }

// //       // Step 2: Free the table via API - try different payload formats
// //       try {
...
// //       // Step 3: Clear ALL frontend state completely - THIS IS CRITICAL
...
// //       // CRITICAL: Reset session tracking so old orders won't show
// //       setSessionId("");
...
// //       // alert(
// //       //   `✅ Payment successful! Thank you for your order.\n\nTable ${currentTableNumber} has been freed.\n\n${currentCustomerId}, you can now scan the QR code again to start a fresh order!`
// //       // );
// //     } catch (error) {
// //       console.error("Failed to complete payment process:", error);

// //       // CRITICAL: Always clear the state even if backend fails
// //       setCart([]);
// //       setCustomerId("");
// //       setPaymentQR(null);
// //       setMyOrders([]);
...
// //       // alert(
// //       //   "Payment completed! Session has been cleared.\nYou can scan a new QR code to start fresh."
// //       // );
// //     }
// //   };
...
// //                     <p className="font-semibold">Order {order.items.name}</p>
...
// const API_BASE_URL = "http://localhost:5000/api";
// const SESSION_KEY = "quickorders_session"; // localStorage key
...
//   const [stage, setStage] = useState<
//     "qr-scan" | "menu" | "cart" | "order-tracking" | "bill" | "payment"
//   >(savedSession?.stage || "qr-scan");

//   const [tableNumber, setTableNumber] = useState<number | null>(
//     savedSession?.tableNumber || null
//   );
//   const [customerId, setCustomerId] = useState<string>(
//     savedSession?.customerId || ""
//   );
...
//   // Save session whenever critical data changes
//   useEffect(() => {
//     if (tableNumber && stage !== "qr-scan") {
//       const sessionData = {
//         stage,
//         tableNumber,
//         customerId,
...
//     }
//   }, [stage, tableNumber, customerId, cart, sessionStartTime]);
...
//       // Step 3: Clear ALL frontend state and localStorage
//       clearSession();
//     } catch (error) {
//       console.error("Failed to complete payment process:", error);
...
//   // All other stages remain exactly the same as your original code
//   // (menu, cart, order-tracking, bill, payment)
//   // ... [rest of your original code unchanged] ...

//   // (I kept the rest of your code 100% identical - only added persistence above)

//   // MENU STAGE (unchanged)
//   if (stage === "menu") {
//     if (loading) {
//       return (
//         <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center">
...
//   // CART STAGE (unchanged - only copied)
//   if (stage === "cart") {
//     const total = calculateTotal();
...
//   // ORDER TRACKING STAGE (unchanged)
//   if (stage === "order-tracking") {
//     return (
//       <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
//         {/* Header */}
...
export default function CustomerApp({ onBack }: { onBack?: () => void }) {
  ...
  const validateTable = (tableNum: number): boolean => {
    if (!tables) return false;
    return tables.some((t) => t.tableNumber === tableNum);
  };
        ...
        if (code) {
          ...
          try {
            ...
            if (tableParam) {
              ...
              // NEW: Validate table exists in database
              if (!validateTable(tableNum)) {
                toast.error(
                  "Table not found. Please scan a valid table QR code."
                );
                return;
              }
              ...
              setStage("menu");
              setCameraActive(false);
              toast.success(`Welcome to Table ${tableNum}!`);
            } else {
        ...
        if (code) {
          ...
          try {
            ...
            if (tableParam) {
              ...
              // NEW: Validate table exists
              if (!validateTable(tableNum)) {
                toast.error(
                  "Table not found. Please use a valid table QR code."
                );
                return;
              }
              ...
              console.log("New session started:", newSessionId);
              setStage("menu");
              toast.success(`Table ${tableNum} connected!`);
            } else {
              toast.error("Invalid QR code. No table number found.");
            }
          } catch (err) {
  ...
  // All other stages (menu, cart, order-tracking, bill, payment) remain 100% unchanged
  // ... [Your original code for all other stages is copied exactly below] ...
  ...
  // CART, ORDER-TRACKING, BILL, PAYMENT stages remain 100% unchanged from your original code
...
// const API_BASE_URL = "http://localhost:5000/api";
// const SESSION_KEY = "quickorders_customer_session"; // localStorage key
...
//   // Clear session only after payment
//   const clearSession = () => {
//     localStorage.removeItem(SESSION_KEY);
//     setStage("qr-scan");
//     setTableNumber(null);
...
//     setMenuItems([]);
//     setEsewaPaymentData(null);
//     toast.success("Thank you! You can now scan a new table to order again.");
//   };
...
//   const handleCheckout = async () => {
//     if (!customerId.trim()) return alert("Please enter your name");
...
//   const handleEsewaPayment = async () => {
//     if (!tableNumber || !customerId) return alert("Missing info");

//     setPaymentLoading(true);
//     try {
//       const grandTotal = calculateGrandTotal();
//       const res = await axios.post(`${API_BASE_URL}/payments/initiate`, {
//         tableNumber,
//         customerId,
//         grandTotal,
//       });
...
//       if (res.data.success) {
//         setEsewaPaymentData(res.data.paymentData);
//         setTimeout(() => esewaFormRef.current?.submit(), 200);
//       }
//     } catch {
...
//   );
//   const allOrdersServed =
//     myOrders.length > 0 && myOrders.every((o) => o.status === "served");
...
//   // MENU, CART, ORDER-TRACKING, BILL, PAYMENT — all exactly same as your original
//   // (No changes made to any UI code)
...
//   // Remaining stages (cart, order-tracking, bill, payment) are exactly your original code
//   // No changes made

//   // ... [All other stages copied verbatim from your original code]
...
//   // Order Tracking, Bill, Payment stages — unchanged
//   // (your original code continues here exactly as before)

//   // ... [rest of your original code for order-tracking, bill, payment stages]
