...
export default function KitchenDashboard({ onBack }: { onBack?: () => void }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      {/* Header */}
      <div className="sticky top-0 z-20 backdrop-blur-lg bg-slate-900/90 border-b-4 border-amber-500 shadow-2xl">
        <div className="max-w-7xl mx-auto px-6 py-8">
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-6">
              <div className="w-20 h-20 bg-gradient-to-br from-orange-500 to-red-600 rounded-3xl flex items-center justify-center text-5xl shadow-2xl">
                👨‍🍳
              </div>
              <div>
            ...
            <div className="flex items-center gap-6">
              {/* <button
                onClick={fetchOrders}
                className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-black rounded-xl shadow-lg transition"
              >
                🔄 Refresh Now
              </button> */}
              ...
              <div className="relative">
                <button
                  ...
                  className="w-14 h-14 bg-gradient-to-br from-amber-500 to-orange-600 text-white font-black text-2xl rounded-full flex items-center justify-center shadow-2xl hover:scale-110 transition"
                >
                  {avatarLetter}
                </button>
                ...
                {showDropdown && (
                  <>
                    <div className="absolute right-0 mt-4 w-64 bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden z-50">
                      <div className="px-6 py-5 border-b border-slate-200 text-center">
                        ...
                        <p className="text-xs text-amber-600 font-bold mt-2 uppercase tracking-wide">
                          Kitchen Staff
                        </p>
                      </div>
                      <button
                        onClick={handleLogout}
                        className="w-full text-left px-6 py-5 hover:bg-red-50 text-red-600 font-bold text-lg transition"
                      >
                        🚪 Logout
                      </button>
                    </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            <div className="bg-gradient-to-br from-red-600/30 to-red-800/10 border border-red-500/40 rounded-3xl p-8 backdrop-blur-sm">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-lg font-black text-red-300 uppercase mb-3">
                    Pending
                  </p>
            ...
            <div className="bg-gradient-to-br from-amber-600/30 to-amber-800/10 border border-amber-500/40 rounded-3xl p-8 backdrop-blur-sm">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-lg font-black text-amber-300 uppercase mb-3">
                    Cooking
                  </p>
            ...
            <div className="bg-gradient-to-br from-green-600/30 to-green-800/10 border border-green-500/40 rounded-3xl p-8 backdrop-blur-sm">
              <div className="flex justify-between items-center">
                <div>
                  <p className="text-lg font-black text-green-300 uppercase mb-3">
                    Ready
                  </p>
          ...
          <p className="text-center text-slate-400 font-semibold">
            🔄 Auto-refresh every 5 seconds • Only active orders shown
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
            {sortedOrders.map((order) => {
              ...
              return (
                <div
                  key={order._id}
                  className={`bg-gradient-to-br ${config.gradient} ${config.border} border-4 rounded-3xl p-8 shadow-2xl hover:shadow-3xl transition-all`}
                >
                  <div className="flex justify-between items-center mb-6">
                    <h2 className={`text-5xl font-black ${config.text}`}>
                      TABLE {order.tableNumber}
                    </h2>
                  ...
                  <div className="flex flex-wrap gap-3 mb-6">
                    <span
                      className={`px-4 py-2 rounded-full text-sm font-black ${config.badge}`}
                    >
                      {config.label}
                    </span>
                    <span className="px-4 py-2 bg-slate-800/50 rounded-full text-sm text-slate-300 font-bold">
                  ...
                  <div className="space-y-4 mb-8">
                    {order.items.map((item, i) => (
                      <div key={i} className="bg-slate-800/60 rounded-2xl p-5">
                        <div className="flex justify-between items-center">
                          <div>
                  ...
                  <div className="border-t border-slate-600/50 pt-6 mb-8">
                    <div className="flex justify-between items-center">
                  ...
                  <button
                    ...
                    className={`w-full bg-gradient-to-r ${config.button} text-white font-black text-xl py-5 rounded-2xl shadow-xl hover:shadow-2xl transition`}
                  >
                    {config.next === "preparing" && "🍳 Start Cooking"}
                    {config.next === "ready" && "✓ Mark as Ready"}
                    {config.next === "served" && "🎉 Mark as Served"}
                  </button>

                  {order.status === "pending" && (
                    <button
                      onClick={() => cancelOrder(order._id)}
                      className="w-full mt-4 bg-slate-700 hover:bg-slate-600 text-white font-bold py-4 rounded-2xl transition"
                    >
                      ✕ Cancel Order
                    </button>
                  )}
